...
/** A drawn chevron, not a `‹` glyph: the text character renders thin and hard to see at
 *  every size that fits. Decorative — both buttons already carry a name. */
...
/** A collapsible, drag-resizable column: a pointer handler and a number in the parent's
 *  state, rather than a resizable-panel dependency for thirty lines of behaviour. */
export default function SidePanel({
  ...
  // Pointer capture keeps move/up coming to the handle when the cursor outruns it, so
  // there are no window listeners and no "am I dragging" flag to leak.
  const onPointerDown = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    // Capture can be lost without a pointerup, and the listeners would stay bound to a
    // closure holding a stale `startWidth` — so the next drag jumps by the old delta.
    ...
    >
      <div className="panel flex min-h-0 min-w-0 flex-1 flex-col gap-1.5 overflow-hidden p-2.5">
        <div className={`flex ${side === "left" ? "justify-end" : "justify-start"}`}>
          {/* Bordered and labelled rather than a bare glyph: a collapse control nobody
              finds is one nobody uses. */}
          <button
            type="button"
            aria-expanded={true}
      ...
      <div
        ...
        // Visible at rest: a drag target you find by sweeping the mouse is not one.
