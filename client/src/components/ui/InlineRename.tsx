...
/**
 * One inline editor for both lists, rendered in place of the row it edits, so a duplicate-name
 * error lands under the field rather than in the banner at the other end of the app.
 *
 * Blur deliberately does nothing: committing on blur races the Save button (the click blurs
 * first), and cancelling on blur throws typing away. Four explicit exits instead — Enter,
 * Escape, Save, Cancel.
 */
export default function InlineRename({ value, label, onSubmit, onCancel }: InlineRenameProps) {
  ...
  const submit = async () => {
    ...
    setPending(true);
    setError(null);
    // A rejected rename keeps the row open with what the user typed.
  ...
  return (
    ...
    >
      <input
        ...
        onKeyDown={(event) => {
          if (event.key === "Escape") {
            // Stopped here, or the same Escape reaches the dialog layer's listener.
