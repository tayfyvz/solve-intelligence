...
export interface NewPatentDialogProps {
  /**
   * Creates the patent. Resolves to an error sentence — a duplicate title is a
   * 409 whose detail already reads as English — or null on success, in which case
...
/**
 * Title required, starting content optional. The error stays *in* the dialog with
 * the fields still filled in: a rejected title is one word away from a good one,
 * and closing the dialog would throw away the content the user just pasted.
 */
    ...
    >
      ...
      >
        ...
        <input
          ...
          autoFocus
          required
          // Matches the server's limit, so a long paste is trimmed here rather
          // than coming back as a 422 the user cannot act on.
          maxLength={200}
          value={title}
          disabled={pending}
