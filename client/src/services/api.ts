...
/** The only place that knows the base URL, the timeouts, and how to turn an unknown
 *  throwable into a sentence a user can read. No React and no store imports, so a caller can
 *  mock this module with a single `vi.mock`. */
export const BASE_URL = import.meta.env.VITE_API_URL ?? "http://localhost:8000";

const http = axios.create({ baseURL: BASE_URL, timeout: 15_000 });
// Longer than the server's own 75 s AI budget, or the client reports a timeout for a
// request that actually succeeded.
const aiHttp = axios.create({ baseURL: BASE_URL, timeout: 90_000 });

/** Carries the HTTP status alongside the message: callers need to tell "AI is not
 *  configured" (503) from any other failure, and `toMessage` erases that. */
...
/** 204 on success. A 409 means the server refused to delete a document's only remaining
 *  version; its `detail` is already a readable sentence. */
...
/**
 * Run 1. Returns one of six outcomes; `status: "error"` is a 200 with a message, not an
 * exception, so this throws only on transport or HTTP failures.
 *
 * `body.html` must be `editor.getHTML()`, never a version's stored `content`: the server
 * hashes exactly these bytes into `proposal.base_sha256`, and hashing two normalisations of
 * one document makes every proposal 409.
 */
...
/**
 * Run 2. Deterministic and offline: it never reaches OpenAI, so it works with no API key. A
 * 409 means the document drifted since the proposal was written — ask again, never retry.
 *
 * On `aiHttp` (90 s) only so both halves of one interaction fail with the same wording; this
 * call is CPU-bound and returns in milliseconds.
 */
...
/** Transient upstream failures, worth re-sending unchanged. Not 503 (configuration) and not
 *  409 (the document moved, so the proposal must be re-asked). */
