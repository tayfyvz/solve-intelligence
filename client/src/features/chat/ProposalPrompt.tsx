...
/**
 * The confirmation, inline in the transcript and never a modal: a modal would cover the
 * document the user needs to read in order to answer. It carries no HTML — the preview is
 * the server's own per-operation summary, so what is approved is what was shown.
 */
export default function ProposalPrompt({
  ...
  return (
    // Tinted in the accent: the one place in the transcript where a click changes the
    // document, so it has to be findable at a glance in a long scroll-back.
    <div className="mt-2 rounded-xl border border-indigo-200 bg-gradient-to-b from-indigo-50/80 to-white p-3 shadow-sm">
      {/* The most useful thing to know before clicking: does this write new claim
          language, or only rearrange text the user wrote? */}
      ...
      {/* What Proceed actually does, and the answer to "why am I being asked?". */}
