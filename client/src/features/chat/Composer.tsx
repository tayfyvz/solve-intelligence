...
/** The server's own cap, enforced here so a long paste fails before the round trip — by the
 *  time the 422 arrives the composer has been cleared and the text is gone. */
...
export default function Composer({
  ...
  return (
    // A tinted footer, so the composer reads as a fixed surface rather than the last item.
    <div className="rounded-xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-3 shadow-sm">
      ...
      <button
        type="button"
        onClick={onSend}
        // An empty composer cannot send, so an attached file is never an instruction.
