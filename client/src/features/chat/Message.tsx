...
// `shadow-sm` is per-tone: the system tone has no border and no fill, and a shadow under
// nothing draws a rectangle the reader can see but not explain.
    ...
    <li className={`msg-in flex ${mine ? "justify-end" : "justify-stretch"}`}>
      <div
        className={`min-w-0 rounded-xl border px-3 py-2 text-[0.8125rem] ${
          mine
            ? // Ink, like the primary button: the user's own turn never needs re-reading,
              // so it can be the darkest object and still the least demanding.
      ...
      >
        {/* whitespace-pre-line: bubbles carry a blank line between the server's sentence
            and ours. */}
        <p className="whitespace-pre-line break-words">{message.text}</p>

        {/* Warnings are the highest-value output here, so they are not a footnote. */}
        ...
        {/* Last message only: a stale question's options must not stay clickable, and a
            Retry further up would re-send something the user has moved past. */}
