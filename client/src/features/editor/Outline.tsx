...
import { documentOutline, scrollToPosition, type OutlineEntry } from "./navigate";
...
/** Above this, a claim group starts collapsed: sixty claims must not bury five headings. */
...
/** A heading and the claims under it, so the outline reads like the document. */
interface Group {
...
/**
 * The document map: every heading and every claim under it, each a button that scrolls the
 * editor to it. Deliberately not a page view — it reads `editor.state.doc` and calls
 * `scrollIntoView`, so selection, undo and claim resolution are untouched.
 */
export default function Outline() {
  ...
  const jump = (from: number, to: number) => {
    ...
    // Cosmetic only: `paint` dispatches a meta-only transaction, so `docChanged` is false.
    ...
    <section aria-labelledby="outline-heading" className="flex min-h-0 flex-col">
      ...
      {all.length === 0 ? (
        ...
        <>
          ...
          {/* Bounded and scrollable: sixty claims must not push the tree off the rail. */}
          <nav
            ...
            {visible.length === 0 ? (
              ...
              <ul className="space-y-0.5">
                {sections.map((group, index) => {
                  // A filtered list is always open; otherwise the user's toggle wins, and
                  // failing that the size rule decides.
                  const open =
                    needle !== "" ||
