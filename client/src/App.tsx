...
import DirtyDialog from "./features/versions/DirtyDialog";
import Editor from "./features/editor/Editor";
import FindBar from "./features/editor/FindBar";
import ImportPatentDialog from "./features/patents/ImportPatentDialog";
import NewPatentDialog from "./features/patents/NewPatentDialog";
import Outline from "./features/editor/Outline";
import PatentTree from "./features/patents/PatentTree";
import SidePanel from "./components/layout/SidePanel";
import ChatPanel from "./features/chat/ChatPanel";
import { useDocumentStore } from "./store";

/** What the user asked for next, held back by the dirty dialog. Creating and importing are in
 *  here because the store opens the result, replacing what is in the editor. */
...
/** Store writes report failure as `false` and leave the sentence in `error`. Inline editors
 *  want it next to the field, so this hands it back and clears the shared slot. */
async function messageOnFailure(run: () => Promise<boolean>): Promise<string | null> {
  const ok = await run();
  if (ok) return null;
  const { error, clearError } = useDocumentStore.getState();
  // No message means the write was discarded as stale: a later request owns the screen, so
  // claiming a failure here would report one that did not happen.
...
function useIsWide(): boolean {
  ...
  // Optional-chained because jsdom has no matchMedia: no layout query falls back to the
  // stacked layout rather than crashing.
...
/** App owns the layout, the mount-time load, the window drag guards, the error banner, the
 *  editor's remount key and the only dirty guard. Everything below it takes props. */
export default function App() {
  ...
  // One value for every control that changes what is in the editor. If the lists and the
  // header disagreed, a save in flight would leave the tree clickable and open a dirty dialog
  // whose buttons are all disabled. Paging is not in here; it never touches the editor.
  const busy = loading || saving;

  // All local: nothing else reads them.
  ...
  // Without these, dropping a .txt outside the drop zone navigates the browser to the file and
  // destroys unsaved work. Here rather than in ChatPanel, which remounts mid-drag.
  useEffect(() => {
  ...
  // The dirty dialog covers switching; a reload or a closed tab drops the same edits.
  // Registered only while dirty, or it would prompt on every exit.
  useEffect(() => {
    if (!dirty) return;
  ...
  const request = (target: PendingSelection) => {
    ...
    // The dialog starts clean: a leftover error would read as if the save the user has not
    // yet asked for had already failed.
    clearError();
    setPending(target);
  };

  /**
   * The held-back action proceeds the moment the document stops being dirty, whichever save
   * button cleared it. One rule instead of a per-button `if (ok) commit(...)`, and it gets the
   * awkward cases free: a cancelled save finds no pending selection, and a failed save leaves
   * `dirty` set, so the dialog stays open with the reason inside it.
   */
  useEffect(() => {
  ...
  return (
    <div className="flex h-full w-full flex-col">
      ...
      {/* Columns above `lg`, stacked and scrolling below it. */}
      <div className="flex min-h-0 flex-1 flex-col gap-2.5 overflow-y-auto p-2.5 sm:p-3 lg:flex-row lg:gap-1 lg:overflow-hidden">
        ...
        >
          ...
          />
          {/* A sibling of the tree, not a tab: on a long patent the outline is how you move. */}
          <div className="border-t border-slate-200 pt-1.5">
            <Outline />
          </div>
        </SidePanel>

        {/* overflow-hidden, not overflow-y-auto: the document scrolls in the editor inside this
            cell, so the find bar and the error banner stay pinned. */}
        <main className="panel flex min-h-[26rem] min-w-0 flex-1 flex-col overflow-hidden lg:min-h-0">
          {/* The dirty dialog renders the shared `error` itself, so the banner stands down
              rather than saying it twice. The other dialogs render only their own submit's
              message, so the banner must stay live while they are open. */}
          {error && !pending && (
            <div
              role="alert"
          ...
          {/* `content !== null`, never `content &&`: "" is a legal emptied version. */}
          {content !== null ? (
            // The editor stays mounted while the next version loads — dimmed and inert, not
            // replaced by a skeleton, so switching is one visual step.
            <div
              aria-busy={loading}
            ...
            >
              {/* Outside the keyed div: remounting would clear a query still in use. It reads
                  the live editor from the store, so it needs no key of its own. */}
              <FindBar />
              {/* Content changes ONLY by remount. This key is the whole mechanism — no sync
                  effect, no setContent. */}
              <div
            ...
            // First paint only. The bars trace a heading and two ragged paragraphs, so the
            // pane loads into roughly the shape it will hold.
            <div
              role="status"
              aria-label="Loading document"
              className="desk flex-1 overflow-hidden px-4"
            >
              {/* The sweep is an overlay, not a background: the bars are opaque, so a gradient
                  behind them would be hidden by the very thing it should light up. */}
              <div className="relative mx-auto mt-6 max-w-[44rem] space-y-3 overflow-hidden rounded-xl border border-slate-200 bg-white p-8 sm:p-14">
                ...
                {/* Last, so `space-y-3` does not give the first bar a stray margin-top. */}
            ...
            <div className="m-auto max-w-xs animate-[rise-in_260ms_cubic-bezier(0.16,1,0.3,1)] text-center text-[0.8125rem] text-slate-500">
              {/* Decorative: an empty pane that is only a sentence reads as a failure. */}
              <div
                aria-hidden="true"
        ...
        <SidePanel
          ...
          resizable={isWide}
        >
          {/* Keyed on the document only: keying on the version would destroy the transcript
              at the instant the user accepts an AI change, including the bubble telling them
              which version it became. The panel clears itself on a user switch instead. */}
          <ChatPanel
            key={documentId ?? "none"}
            documentId={documentId}
            versionNumber={versionNumber}
          ...
          onImport={async (destination, newTitle, newContent) => {
            const message = await messageOnFailure(() =>
              destination === "document"
                ...
                : // No name: the server generates a non-colliding one, so importing the same
                  // file twice cannot 409 on a duplicate version name.
