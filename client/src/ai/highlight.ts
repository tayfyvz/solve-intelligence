  ...
  return new Plugin<DecorationSet>({
    ...
    state: {
      ...
      apply(tr, set) {
        ...
        // Not just clarity: `meta.from` does not exist on the `clear` variant, so
        // deleting this line is a compile error. `tsc` is what guards it — the plugin
        // state cannot tell the two paths apart at runtime (§25.7 correction 26).
