...
| **Vector DB / embeddings** | Still rejected, but the *reason* narrowed once 37-page patents arrived. A whole 112,000-character patent does fit in one call — measured, and faster than a packed context — so for every realistic document there is nothing to retrieve. Above the 120,000-char budget there is, and it is thirty lines of word-overlap scoring over the parsed document: the unit a patent question wants is a **named section**, and a patent already carries its own headings, so paying for an index to rediscover structure the document states is backwards. Chunking a legal document with overlap is still a way to lose the sentence that mattered. |
...
| Seed size vs. real size | Measured the stored content, then a real 37-page patent | ~2.7 KB vs **112,659** — the seeds hid the whole problem. A real patent still fits in one call at a 120,000-char budget (measured: median 2.3 s, *faster* than a 30,000-char packed context), so retrieval only runs above that |
