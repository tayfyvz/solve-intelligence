...
**This route writes nothing**, and has no `db` parameter to write with. It converts and
reports; the client then sends the result to the existing `POST /api/documents` or
`POST /api/documents/{id}/versions`, so import gets version creation, naming,
sanitising, size limits and 409 handling for free rather than a second copy of each.

It is a preview in the literal sense: `content` is byte-for-byte what a save will store,
because the conversion sanitises and canonicalises before returning.
"""
...
def _title_from_filename(filename: str | None) -> str:
    """A last resort, when the text supplies no plausible title of its own. Trimmed of
    its extension and the separators a downloaded file collects, so `US_10123456_B2.txt`
    opens as "US 10123456 B2" rather than "Imported patent"."""
...
def import_text_file(
    ...
    # and then 413s on the way to the database is a preview of something the user can
    # never keep. Checked before any parsing, so a 50 MB paste is cheap.
