"""The Python that guards the understanding. Pure: no `openai`, no `langgraph`, no I/O.
...
from app.ai.document import ParsedDocument
...
class Turn(Protocol):
    """The shape `clarify_floor` needs, and no more. A structural type rather than an
    import of `app.schemas.ChatTurn`: that is the wire layer, and an engine module that
    imports it inverts the dependency for the sake of two attributes."""
...
# `claim3` has no word boundary between the word and the digit, so a trailing \b would be
# wrong here. A negative lookahead for a letter keeps "claimed" out.
...
def collapse(text: str) -> str:
    ...
    and rewriting the instruction would make the restatement a restatement of our guess.
    """
...
def _resolved(intent: str, restatement: str, numbers: list[int]) -> Understanding:
...
def fast_understanding(
    ...
    """A deterministic understander for three unambiguous sentences. Not a router: it
    returns a fully resolved Understanding, or None.

    It refuses to fire while a question is pending (an answer is data, not a fresh
    instruction), and on a claim number this parse does not contain. Anchoring makes it
    fail CLOSED, so "mak claim3 bold" falls through to the model, correctly.
    """
    if pending_question:
        return None
    ...
    if match is not None:
        ...
        if number not in numbers:
            return None
        return _resolved("edit_ops", f"Make claim {number} {mark}.", [number])
    ...
    if match is not None:
        ...
        if number not in numbers:
            return None
        restatement = (
            f"Remove {mark} from claim {number}."
            if text.lower().startswith("un")
            else f"Make claim {number} {mark}."
        )
        return _resolved("edit_ops", restatement, [number])
    ...
    if match is not None:
        ...
        if number not in numbers:
            return None
        return _resolved("edit_ops", f"Delete claim {number}.", [number])

    return None


# Public: `nodes._retrieve` reads this too, as the backstop for a file the model was told
# about but did not flag as relevant.
...
def missing_file_question(instruction: str, prior_art: str) -> str | None:
    ...
    "file" for another reason to attach one. It counts against `clarify_count` like any
    other clarification, or it would be a loop with no bound.
    """
...
def resolve_outcome(u: Understanding, *, clarify_count: int) -> Understanding:
    """Turn an unresolved understanding into a TERMINAL outcome once the budget is spent.
    ...
    Terminal is the point. Rewriting `question` while leaving `resolved=False` still
    produces status="needs_clarification" on the wire, the client stores it as a pending
    question, the next turn clamps straight back to the ceiling, and the user is handed
    the same sentence forever. A bound that does not change the OUTCOME is not a bound.
    """
    if u.resolved or clarify_allowed(clarify_count):
        # Overwritten on BOTH paths: strict Structured Outputs makes the model emit this
        # field, and an early `return u` would let a model that guessed True terminate a
        # turn Python never terminated.
...
def clarify_floor(history: list[Turn]) -> int:
    ...
    that suppresses the questions to lower this floor also removes the context that makes
    its own conversation work. Lying costs the liar.
    """
    seen = 0
...
def gate_understanding(
    ...
    Runs on every path, including the fast path, and BEFORE any branch is chosen.
    """
    numbers = {c.number for c in doc.claims}

    # 1. Low confidence never acts. A model saying confidence="low", resolved=True is
    #    self-contradictory, and the contradiction is exactly what a prompt-only rule
    #    fails on. The reverse — high confidence, resolved=False — is left alone: an
    #    honest question is always allowed.
    if u.confidence == "low":
        u = u.model_copy(update={"resolved": False, "question": u.question or WHICH_CLAIM})

    # 2. Every resolved claim number must exist in THIS parse. Running before the branch
    #    is what makes a nonexistent number unable to REACH plan_ops or draft, rather
    #    than merely being rejected by them.
    ...
    # 5. Options belong to a question. A real call returned a resolved, high-confidence
    #    understanding WITH a populated options list, which the panel would have rendered
    #    as clarification buttons under a bubble saying the work was done.
