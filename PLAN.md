...
| 38 | 6C | §28.2.2's production client stage sets `ARG VITE_API_URL=/api` as the same-origin default. | **Wrong for this client, and silently so.** `api.ts` reads `VITE_API_URL` as an **origin** (`?? "http://localhost:8000"`) and every path it requests already begins with `/api/`. Building with `/api` makes axios request `/api/api/ai/chat` — every call 404s, and only in the production image, which is the one nobody runs before submitting. | `ARG VITE_API_URL=""`. An empty origin makes axios issue relative requests against whatever host served the page, which is exactly the same-origin behaviour §28.2.2 wanted. `""` survives `??` (it is not nullish), so the localhost default still applies to `npm run dev`. **Verified live**: the production image serves the SPA, `/api/health` proxies through nginx, a deep link falls back to `index.html`, the bundle carries no `@vite/client`, and a 3 MB POST is refused **413 by nginx** before FastAPI reads it (§28.2.4). |
| 39 | 6C | §28.4(a): "Ship the dev CMD as the compose override, same pattern as the server" — for the **client**. | The pattern does not transfer. The server is one image with two commands, so a `command:` override works. The client's production image is `nginx:alpine`: it contains no node, no source and no `package.json`, so no `command:` can make it run Vite. | Two **targets**, not two commands. `client/Dockerfile` declares `dev` → `build` → `production`; the default target is the last stage, so a plain `docker build ./client` is production, and compose selects development with `build: {context: ./client, target: dev}`. Same principle — dev and prod differ by what compose selects, never by a branch in the application — expressed with the mechanism the client image actually has. |

| 41 | 6E | §27.1 rows 2, 13, 15, 16, 18 and the §23.8 scenario are marked **Manual** with no statement of *where the result is recorded*, and §27's preamble says only "recorded in the submission notes". | There were no submission notes, and a manual row whose result lives nowhere is indistinguishable at review time from a manual row nobody walked. | **`PHASE6-MANUAL.md`**, committed at 6E: every manual row with its observed result, the four defects the walk found that no test could, and an explicit **"Not run"** section. Three rows are recorded as passing for a *weaker* reason than the plan implies — row 18 (DevTools throttling shapes the browser↔server hop only, not server↔OpenAI), the StrictMode row (behavioural evidence; `store.editor` is not reachable from the page), and row 15 (three refusals are not prevention). |
| 42 | 6E | `insert_section`'s vocabulary line in `PLAN_SYSTEM`/`DRAFT_SYSTEM` offers `position: before_claims\|after_claims` and says nothing about which to use. §1.2 reshape 1 and C8 settled what the anchors *mean*, never which one a given section *wants*. | README acceptance example 4 put the **Background section after the claims**. Structurally valid, verified fine, and wrong for a patent — the Background is part of the specification and precedes the claims. | Both prompts annotate the anchor: Background/Field/Summary/Description of Related Art → `before_claims`; `after_claims` is for the few sections that follow. Re-walked: the proposal now reads *"before the claims"* and the section lands above `<h1>Claims</h1>`. |
| 43 | 6E | `JUDGE_SYSTEM` opens *"You review one proposed patent claim edit"* and applies check 1 (CLAIM FORM) to whatever it is shown. | It is shown **every** operation, including `insert_section` — which is specification prose by definition. A correct Background section was returned with *"Reviewer note: Not in claim form: … not a single-sentence patent claim."* A false alarm on the user's screen teaches them to ignore reviewer notes, which is the opposite of what the judge is for. | A SCOPE FIRST paragraph: checks 1/2/3/5 apply to `insert_claim` and `replace_claim` only; an `insert_section` is judged on check 4 (terminology) alone. |
| 44 | 6E | `ANSWER_SYSTEM` rule 2: *"A citation quotes a short span VERBATIM **from the material below**."* | "The material below" includes the scaffolding `build_context`/`claims_excerpt` add — `--- CLAIMS (9) ---` rules and `[4]` claim-number prefixes — none of which is in the user's document. Asking *"How many claims does this patent have?"* produced a **correct** answer carrying **two amber warnings**, because `check_citations` correctly found the quoted scaffolding absent from the document. The checker was right; the prompt was wrong. | Rule **2a**: quote the document's own words only, naming the scaffolding explicitly and saying why (a quote containing it fails the automatic check and warns the user about an answer that was correct). Re-walked over three questions: zero warnings. |
| 40 | 6E | **`CLAUDE.md` "Verified environment facts" and §20.7/§21.2:** `temperature` IS accepted on `gpt-5.2-2025-12-11` (measured 0.0, 1.0, 2.0 at 4Z), and `reasoning_effort="low"` is accepted. Config shipped `openai_reasoning_effort="low"`; §21.2 sends `temperature=0` on `understand`, `plan_ops` and `judge`. | **Both facts are individually true and the shipped combination is rejected.** `gpt-5.2-2025-12-11` refuses `reasoning_effort` together with any `temperature` other than the default: `400 Unsupported value: 'temperature' does not support 0.0 with this model. Only the default (1) value is supported.` Re-measured 2026-08-14: effort+no-temp → OK · effort+temp=1.0 → OK · no-effort+temp=0.0 → OK · **effort+temp=0.0 → 400**. 4Z varied the two parameters **independently** and never tried them together. Consequence: **three of the five nodes 400 on every live call and the AI feature was entirely non-functional in the browser** — the first real click-through of Step 6 hit it immediately. Nothing in the suite could catch it, because by design no test makes a live call; 4Z was the only live gate. | **`openai_reasoning_effort` default → `None`** (agreed with the author before deviating). `reasoning_effort` is the parameter that goes, on 4Z's own evidence: `reasoning_tokens == 0` on all 14 measured calls, so it buys nothing measurable on this model, whereas `temperature=0` is what makes the same instruction resolve the same way twice — the whole of §21.2's deterministic/generative split. Guarded by **L11** (configuration: effort set ⇒ no non-default temperature anywhere) and **L12** (the wire: the shipped call omits `reasoning_effort` and sends `temperature=0`). Both verified red by restoring `"low"`. `CLAUDE.md`'s verified-facts block corrected in the same commit. |
...
- [x] **Production CMDs written down and the dev override moved to compose** — DONE (6C). (§28.2.1, §28.2.2):
...
- [x] **`client/Dockerfile` production stage BUILT AND VERIFIED** — DONE (6C). with the exact multi-stage build, the
